...
# É uma boa prática usar variáveis de ambiente para tokens e URLs sensíveis
API_URL = os.environ.get("API_URL", "https://api.asafebroker.com/admin-token/deposits")
API_TOKEN = os.environ.get("API_TOKEN", "o7efkbcw58") # Use uma variável de ambiente real em produção!
...
@app.route("/data")
def data():
    # Pegando parâmetros da query string, com defaults e validação básica
    try:
    ...
    try:
        ...
        response = requests.get(API_URL, headers=headers, params=params, timeout=10) # Adicionado timeout
        response.raise_for_status()  # Levanta erro em caso de status HTTP 4xx ou 5xx
        data = response.json()
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port, debug=True) # Mantenha debug=True para desenvolvimento
